...
console.log("🚀 Démarrage main_v2.js (Correctif limitMin)...");
...
function startFlashingEffect(event) {
    ...
    flashTimer = setInterval(() => {
        ...
        const chart = charts['eventsTimelineChart'];
...
function spawnVehicle(eventType, dba) {
    ...
    if (!vehicleConfig || eventType === 'Autre') {
        const randV = randomVehicles[Math.floor(Math.random() * randomVehicles.length)];
        vehicleConfig = { icon: randV.icon, color: randV.color, isEmergency: false };
    }
    ...
    vehicleIcon.className = `fa ${vehicleConfig.icon} moving-vehicle fa-flip-horizontal`;
    vehicleIcon.style.color = vehicleConfig.color;

    if (vehicleConfig.isEmergency || ['fa-ambulance', 'fa-truck-medical', 'fa-car-side'].includes(vehicleConfig.icon)) {
        if (Math.random() > 0.5) vehicleIcon.classList.add('emergency-light');
    }

    const bubble = document.createElement('div');
    bubble.className = 'comic-bubble';
    bubble.innerText = Math.round(dba) + " dB";
    let scale = 1 + (Math.max(0, dba - 50) / 50);
    scale = Math.min(scale, 2.5);
    bubble.style.transform = `scale(${scale})`;

    wrapper.appendChild(bubble);
    wrapper.appendChild(vehicleIcon);
...
function updateKPIs(kpis) {
    ...
    function generateTrendHTML(kpi_data, period, decimals, transform) {
        ...
        return `<span class="kpi-long-term-trend ${trendClass}">
    ...
    if (kpis.temperature_c) {
        previousKPIs = {
            ...
            pressure_pa: kpis.pressure_pa.value,
            aqi: kpis.aqi.value,
...
function createSensorChart(canvasId, label, dataKey, color, unit, historyData, period, transformFunc = v => v, isLog = false) {
    ...
    const datasets = [{
        label: label, data: chartData, borderColor: color, backgroundColor: color + '33',
        borderWidth: 2, pointRadius: 0, tension: 0.1, fill: false, spanGaps: false
    }];
    ...
    if (historyData[0] && rollingMeanKey in historyData[0]) {
        ...
        if (rollingData.length > 0) {
            datasets.push({
                label: 'Tendance', data: rollingData, borderColor: '#ffffff',
                borderWidth: 2, pointRadius: 0, tension: 0.4, borderDash: [], fill: false, spanGaps: true
            });
        }
    }
    ...
    charts[canvasId] = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { type: 'time', time: { unit: timeUnit, displayFormats: { hour: 'HH:mm', day: 'dd/MM' } }, grid: { color: '#3e3e3e' }, ticks: { color: '#b8c7ce' } },
...
// --- FRISE (CORRECTIF LIMITMIN -> MINTIME) ---
// --- FRISE (Avec Flash ROUGE) ---
function createEventsTimelineChart(canvasId, eventsData, period) {
    ...
    const datasets = styleKeys.map(eventType => {
        ...
        return {
            label: eventType, data: data, backgroundColor: style.color,

            // --- MODIFICATION ICI : FLASH ROUGE ---
            borderColor: function (context) {
                if (context.raw && context.raw.x === flashTargetTime && flashState) {
                    return '#FF0000'; // ROUGE VIF
                }
                return 'rgba(255, 255, 255, 0.8)'; // Bordure normale blanche
            },

            borderWidth: function (context) {
                if (context.raw && context.raw.x === flashTargetTime && flashState) return 4;
                return 1;
            },

            pointStyle: style.style, pointRadius: getRadius, pointHoverRadius: (c) => getRadius(c) + 8
        };
    }).filter(ds => ds !== null);
    ...
    charts[canvasId] = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                zoom: { pan: { enabled: true, mode: 'x', modifierKey: null }, zoom: { wheel: { enabled: true }, pinch: { enabled: true }, mode: 'x' }, limits: { x: { min: minTime, max: maxTime } } }
            },
            scales: { x: { type: 'time', min: minTime, max: maxTime, time: { unit: timeUnit, displayFormats: { hour: 'HH:mm', day: 'dd/MM' } }, grid: { color: '#999', lineWidth: 2 }, ticks: { color: '#fff', font: { size: 14, weight: 'bold' }, maxRotation: 0, autoSkip: true } }, y: { type: 'category', offset: true, grid: { color: '#3e3e3e' }, ticks: { color: '#b8c7ce' } } }, animation: false
        }
    });
}
...
function playAudio(f) {
    ...
    const a = new Audio(); a.src = '/audio_files/' + f; a.crossOrigin = "anonymous"; a.volume = 0.8;
